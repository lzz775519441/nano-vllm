...
class SchedulerTest(unittest.TestCase):
    ...
    def make_scheduler(
        ...
        if max_prefill_chunk_tokens is None:
            max_prefill_chunk_tokens = max_tokens
    ...
    def test_prefill_chunk_uses_separate_token_cap(self):
        scheduler = self.make_scheduler(max_tokens=8, max_prefill_chunk_tokens=4)
        seq = Sequence([1, 2, 3, 4, 5, 6, 7])
        scheduler.add(seq)
    ...
    def test_prefill_chunk_cap_does_not_include_decode_tokens(self):
        scheduler = self.make_scheduler(max_tokens=8, max_prefill_chunk_tokens=4)
        running = self.make_running(scheduler, [1, 2, 3], 4)
        waiting = Sequence([10, 11, 12, 13, 14])
        scheduler.add(waiting)
